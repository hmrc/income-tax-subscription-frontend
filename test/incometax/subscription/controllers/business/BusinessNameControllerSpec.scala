...
class BusinessNameControllerSpec extends ControllerBaseSpec
  ...
  "Calling the submit action of the BusinessNameController with an authorised user on the sign up journey and valid submission" should {
    ...
    "When it is not in edit mode" should {
      s"return a redirect status (SEE_OTHER - 303) '${incometax.subscription.controllers.business.routes.BusinessAccountingPeriodPriorController.show().url}'" in {
        setupMockKeystoreSaveFunctions()

        val goodRequest = callShow(isEditMode = false)

        status(goodRequest) must be(Status.SEE_OTHER)
        redirectLocation(goodRequest) mustBe Some(incometax.subscription.controllers.business.routes.BusinessAccountingPeriodPriorController.show().url)
  ...
  "Calling the submit action of the BusinessNameController with an authorised user on the registration journey and valid submission" should {
    ...
    "When it is not in edit mode" should {
      s"return a redirect status (SEE_OTHER - 303) redirect to '${incometax.subscription.controllers.business.routes.BusinessPhoneNumberController.show().url}'" in {
        setupMockKeystoreSaveFunctions()

        val goodRequest = callShow(isEditMode = false)

        status(goodRequest) must be(Status.SEE_OTHER)
        redirectLocation(goodRequest) mustBe Some(incometax.subscription.controllers.business.routes.BusinessPhoneNumberController.show().url)
