  ...
  "When registration is enabled" should {
    ...
    "Calling the submit action of the BusinessPhoneNumberController with an authorised user and valid submission" should {
      ...
      "When it is not in edit mode" should {
        ...
        s"redirect to '${incometax.subscription.controllers.business.routes.BusinessAddressController.show().url}'" in {
          setupMockKeystoreSaveFunctions()

          val goodRequest = callShow(isEditMode = false)

          redirectLocation(goodRequest) mustBe Some(incometax.subscription.controllers.business.routes.BusinessAddressController.show().url)
    ...
    "The back url when not in edit mode" should {
      s"point to ${incometax.subscription.controllers.business.routes.BusinessNameController.show().url}" in {
        TestBusinessPhoneNumberController.backUrl(isEditMode = false) mustBe incometax.subscription.controllers.business.routes.BusinessNameController.show().url
      }
    }
