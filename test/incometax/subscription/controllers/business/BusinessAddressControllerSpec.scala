  ...
  "When registration is enabled" should {
      ...
      for (editMode <- Seq(true, false)) {
        s"Edit mode is $editMode" when {
          "There is no address saved in keystore redirect to init" in {
            ...
            redirectLocation(result) mustBe Some(incometax.subscription.controllers.business.routes.BusinessAddressController.init().url)
    ...
    "BusinessAddressController.submit" should {
      ...
      "when editMode is true" in {
        ...
        status(result)
        redirectLocation(result) mustBe Some(incometax.subscription.controllers.business.routes.BusinessAddressController.init(editMode = true).url)
      }

      "when editMode is false" in {
        val result = call(editMode = false)
        status(result)
        redirectLocation(result) mustBe Some(incometax.subscription.controllers.business.routes.BusinessStartDateController.show().url)
      }
    }
          ...
          s"Edit mode is $editMode" should {
            "fetch and persist the address if the call is successful" in {
              ...
              if (editMode)
                ...
                redirectLocation(result).get mustBe incometax.subscription.controllers.business.routes.BusinessStartDateController.show().url
