...
package incometax.subscription.controllers.business
...
class BusinessAccountingPeriodDateControllerSpec extends ControllerBaseSpec
  ...
  "When the user is in the registration journey, BusinessAccountingPeriodDateController" when {
    ...
    "Calling the submit action of the BusinessAccountingPeriodDate with a valid submission" should {
      ...
      "When it is not in edit mode" should {
        "return a redirect status (SEE_OTHER - 303)" in {
          ...
          redirectLocation(goodRequest) mustBe Some(incometax.subscription.controllers.business.routes.BusinessAccountingMethodController.show().url)
    ...
    "The back url" should {
      s"point to ${incometax.subscription.controllers.business.routes.BusinessStartDateController.show().url}" in {
        await(TestBusinessAccountingPeriodController.backUrl(isEditMode = false)(request)) mustBe incometax.subscription.controllers.business.routes.BusinessStartDateController.show().url
      }
    }
  ...
  "When the user is in the sign up journey, BusinessAccountingPeriodDateController" when {
    ...
    "Calling the submit action of the BusinessAccountingPeriodDate with an authorised user and a valid submission" should {
      ...
      "When it is not in edit mode" should {
        ...
        s"redirect to '${incometax.subscription.controllers.business.routes.BusinessAccountingMethodController.show().url}'" in {
          // required for backurl
          setupMockKeystore(fetchAccountingPeriodPrior = TestModels.testIsCurrentPeriod)
    ...
    "The back url when the user is submitting details for current period" should {
      s"point to ${incometax.subscription.controllers.business.routes.BusinessAccountingPeriodPriorController.show().url}" in {
        setupMockKeystore(fetchAccountingPeriodPrior = TestModels.testIsCurrentPeriod)
        await(TestBusinessAccountingPeriodController.backUrl(isEditMode = false)(request)) mustBe incometax.subscription.controllers.business.routes.BusinessAccountingPeriodPriorController.show().url
        verifyKeystore(fetchAccountingPeriodPrior = 1)
      }
    }

    "The back url when the user is submitting details for next period" should {
      s"point to ${incometax.subscription.controllers.business.routes.RegisterNextAccountingPeriodController.show().url}" in {
        setupMockKeystore(fetchAccountingPeriodPrior = TestModels.testIsNextPeriod)
        await(TestBusinessAccountingPeriodController.backUrl(isEditMode = false)(request)) mustBe incometax.subscription.controllers.business.routes.RegisterNextAccountingPeriodController.show().url
        verifyKeystore(fetchAccountingPeriodPrior = 1)
      }
    }
