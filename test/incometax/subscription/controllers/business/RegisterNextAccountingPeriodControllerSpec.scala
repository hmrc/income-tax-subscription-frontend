...
class RegisterNextAccountingPeriodControllerSpec extends ControllerBaseSpec with MockKeystoreService {
  ...
  "Calling the show action of the RegisterNextAccountingPeriod with an authorised user" should {
    ...
    s"The back url should point to '${incometax.subscription.controllers.business.routes.BusinessAccountingPeriodPriorController.show().url}'" in {
      val document = Jsoup.parse(contentAsString(result))
      document.select("#back").attr("href") mustBe incometax.subscription.controllers.business.routes.BusinessAccountingPeriodPriorController.show().url
    }
  }
  ...
  "Calling the submit action of the RegisterNextAccountingPeriod with an authorised user and valid submission" when {
    ...
    "Option 'Yes' is selected" should {
      ...
      s"redirect to ${incometax.subscription.controllers.business.routes.BusinessAccountingPeriodDateController.show().url}" in {
