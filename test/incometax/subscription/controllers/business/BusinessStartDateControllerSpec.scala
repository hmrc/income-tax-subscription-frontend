...
class BusinessStartDateControllerSpec extends ControllerBaseSpec
  ...
  "When registration is enabled" should {
    ...
    "Calling the submit action of the BusinessStartDateController with an authorised user and valid submission" should {
      ...
      "When it is not in edit mode" should {
        ...
        s"redirect to '${incometax.subscription.controllers.business.routes.BusinessAccountingPeriodDateController.show().url}'" in {
          ...
          redirectLocation(goodRequest) mustBe Some(incometax.subscription.controllers.business.routes.BusinessAccountingPeriodDateController.show().url)
    ...
    "The back url" should {
      ...
      s"When it not is not in edit mode, it should point to '${incometax.subscription.controllers.business.routes.BusinessAddressController.show().url}'" in {
        val document = Jsoup.parse(contentAsString(result(option_yes)))
        document.select("#back").attr("href") mustBe incometax.subscription.controllers.business.routes.BusinessAddressController.show().url
      }
