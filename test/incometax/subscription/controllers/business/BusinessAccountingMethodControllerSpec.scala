...
class BusinessAccountingMethodControllerSpec extends ControllerBaseSpec
  ...
  "The back url not in edit mode" should {
    s"point to ${incometax.subscription.controllers.business.routes.BusinessAccountingPeriodDateController.show().url}" in {
      TestBusinessAccountingMethodController.backUrl(isEditMode = false) mustBe incometax.subscription.controllers.business.routes.BusinessAccountingPeriodDateController.show().url
    }
  }
