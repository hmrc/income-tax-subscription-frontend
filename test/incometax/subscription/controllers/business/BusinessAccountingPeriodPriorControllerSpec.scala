...
class BusinessAccountingPeriodPriorControllerSpec extends ControllerBaseSpec with MockKeystoreService {
  ...
  "Calling the show action of the BusinessAccountingPeriodPriorController with an authorised user" should {
    ...
    s"The back url should point to '${incometax.subscription.controllers.business.routes.BusinessNameController.show().url}'" in {
      val document = Jsoup.parse(contentAsString(result))
      document.select("#back").attr("href") mustBe incometax.subscription.controllers.business.routes.BusinessNameController.show().url
    }
  }


  "The back url" should {
    s"point to ${incometax.subscription.controllers.business.routes.BusinessNameController.show().url}" in {
      TestAccountingPeriodPriorController.backUrl mustBe incometax.subscription.controllers.business.routes.BusinessNameController.show().url
    }
  }
  ...
  "Calling the submit action of the BusinessAccountingPeriodPriorController with an authorised user and valid submission" when {
    ...
    "Not in edit mode and " when {
      ...
      "Option 'Yes' is selected and there were no previous entries" in {
        ...
        redirectLocation(goodRequest).get mustBe incometax.subscription.controllers.business.routes.RegisterNextAccountingPeriodController.show().url
      ...
      "Option 'No' is selected and there were no previous entries" in {
        ...
        status(goodRequest) mustBe Status.SEE_OTHER
        redirectLocation(goodRequest).get mustBe incometax.subscription.controllers.business.routes.BusinessAccountingPeriodDateController.show().url
        await(goodRequest)
        verifyKeystore(fetchAccountingPeriodPrior = 1, saveAccountingPeriodPrior = 1)
      }
