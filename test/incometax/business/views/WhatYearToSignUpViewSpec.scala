...
import incometax.business.forms.{AccountingMethodPropertyForm, AccountingYearForm}
...
class WhatYearToSignUpViewSpec extends ViewSpecTrait {
  ...
  "what year to sign up" must {
    ...
    "have content" in new Setup {
      ...
      paragraphs.get(1).text() mustBe messages.example1
      paragraphs.get(2).text() mustBe messages.example2
    }
    ...
    "have a form" which {
      ...
      "has a cash radio button" in new Setup {
        ...
        radioWithLabel.select("label[for=accountingYear-CurrentYear]").text mustBe Seq(messages.option1).mkString(" ")
      }

      "has a accruals radio button" in new Setup {
        val radioWithLabel: Elements = document.select("form div fieldset div.multiple-choice")
        radioWithLabel.select("input[id=accountingYear-NextYear]").`val` mustBe "NextYear"
        radioWithLabel.select("label[for=accountingYear-NextYear]").text mustBe Seq(messages.option2).mkString(" ")
      }
