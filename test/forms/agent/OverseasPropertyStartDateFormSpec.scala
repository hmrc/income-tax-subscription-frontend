  ...
  "The OverseasPropertyStartDateForm" should {
    ...
    "when testing the validation" should {
      "output the appropriate error messages for the start date" when {
        ...
        val invalid = "agent.error.overseas.property.date.empty"
        ...
        "it is missing the day" in {
          ...
          dayTest.errors must contain(FormError(dayKeyError, "agent.error.overseas.property.day.empty"))
        }
        "it is is missing the month" in {
          val mothTest = form.bind(DataMap.date(startDate)("06", "", "2017"))
          mothTest.errors must contain(FormError(monthKeyError, "agent.error.overseas.property.month.empty"))
        }
        "it is is missing the year" in {
          val yearTest = form.bind(DataMap.date(startDate)("06", "4", ""))
          yearTest.errors must contain(FormError(yearKeyError, "agent.error.overseas.property.year.empty"))
        }
        "it is is missing the day and month" in {
          val dayAndMonthTest = form.bind(DataMap.date(startDate)("", "", "2017"))
          dayAndMonthTest.errors must contain(FormError(dayKeyError, "agent.error.overseas.property.day_month.empty"))
        }
        "it is is missing the day and year" in {
          val dayAndYearTest = form.bind(DataMap.date(startDate)("", "4", ""))
          dayAndYearTest.errors must contain(FormError(dayKeyError, "agent.error.overseas.property.day_year.empty"))
        }
        "it is is missing the month and year" in {
          val monthAndYearTest = form.bind(DataMap.date(startDate)("06", "", ""))
          monthAndYearTest.errors must contain(FormError(monthKeyError, "agent.error.overseas.property.month_year.empty"))
        }
      }
    }
