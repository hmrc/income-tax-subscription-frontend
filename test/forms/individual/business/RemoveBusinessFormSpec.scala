  ...
  "RemoveBusinessForm" must {
    "return Yes" when {
      "bound with a yes value" in {
        val boundForm: Form[YesNo] = removeBusinessForm().bind(Map(
    ...
    "return an error" when {
      "bound with an invalid value" in {
        ...
        boundForm.errors mustBe Seq(FormError(fieldName, "error.remove-sole-trader-business.invalid"))
      }
      "bound with no value" in {
        val boundForm: Form[YesNo] = removeBusinessForm().bind(Map.empty[String, String])
