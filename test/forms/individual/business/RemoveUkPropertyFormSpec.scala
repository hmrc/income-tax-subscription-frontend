  ...
  "removeUkPropertyForm" must {
    "return Yes" when {
      "bound with a yes value" in {
        val boundForm: Form[YesNo] = removeUkPropertyForm.bind(Map(
    ...
    "return an error" when {
      "bound with an invalid value" in {
        ...
        boundForm.errors mustBe Seq(FormError(yesNo, "error.remove-uk-property-business.invalid"))
      }
      "bound with no value" in {
        val boundForm: Form[YesNo] = removeUkPropertyForm.bind(Map.empty[String, String])
