    ...
    "when testing the validation" should {
      "output the appropriate error messages for the start date" when {
        ...
        "it is within 1 years" in {
          ...
          maxTest.errors must contain(FormError(dayKeyError, s"$errorContext.day_month_year.maxDate", Seq(OverseasPropertyStartDateForm.maxStartDate.toString)))
        }
        "it is before year 1900" in {
          val minTest = form.bind(DataMap.govukDate(startDate)("31", "12", "1899"))
          minTest.errors must contain(FormError(dayKeyError, s"$errorContext.day_month_year.minDate", Seq(OverseasPropertyStartDateForm.minStartDate.toString)))
        }
