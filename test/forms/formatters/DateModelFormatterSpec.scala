      ...
      s"isAgent is set to $isAgent" should {
        ...
        "return an error" when {
          ...
          "all fields are empty" in {
            ...
            result mustBe Left(Seq(FormError(s"$bindingKey-${DateModelMapping.day}", errorKey(isAgent)("error.test.day_month_year.empty"))))
          }
          ...
          "the date is invalid" when {
            "the date is too early" in {
              ...
              result mustBe Left(Seq(
                FormError(s"$bindingKey-${DateModelMapping.day}", errorKey(isAgent)("error.test.day_month_year.min_date"), List("2022-04-08"))
              ))
            }
            "the date is too late" in {
              ...
              result mustBe Left(Seq(
                FormError(s"$bindingKey-${DateModelMapping.day}", errorKey(isAgent)("error.test.day_month_year.max_date"), List("2020-04-08"))
              ))
            }
          }
