          ...
          "the date is invalid" when {
            "the date is too early" in {
              ...
              result mustBe Left(Seq(
                FormError(s"$bindingKey-${DateModelMapping.day}", errorKey(isAgent)("error.test.day_month_year.minDate"), List("2022-04-08"))
              ))
            }
            "the date is too late" in {
              ...
              result mustBe Left(Seq(
                FormError(s"$bindingKey-${DateModelMapping.day}", errorKey(isAgent)("error.test.day_month_year.maxDate"), List("2020-04-08"))
              ))
            }
          }
