...
import forms.agent.IncomeSourceForm.{incomeSourceKey, overseasPropertyKey, selfEmployedKey, ukPropertyKey}
...
class IncomeSourceViewSpec extends ViewSpec {
  ...
  //
  //  def page(isEditMode: Boolean, addFormErrors: Boolean, foreignProperty: Boolean = true): HtmlFormat.Appendable = incomeSource(
  //    incomeSourceForm = IncomeSourceForm.incomeSourceForm(foreignProperty).addError(addFormErrors),
  //    postAction = action,
  //    backUrl = backUrl,
