...
import assets.MessageLookup.ThrottleEndofJourneyAgent._
...
class ThrottleEndOfJourneyViewSpec extends ViewSpec {
  ...
  lazy val postAction: Call = controllers.agent.routes.TaskListController.show()
  ...
  "business agent end of journey throttle view" must {
    ...
    "have a form that continues" in {
      document1.select("main").select("form").attr("action") mustBe controllers.agent.routes.TaskListController.show().url
    }
