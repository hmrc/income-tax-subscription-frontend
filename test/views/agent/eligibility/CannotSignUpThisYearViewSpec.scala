...
class CannotSignUpThisYearViewSpec extends ViewSpec {
  ...
  val testFormError: FormError = FormError(yesNo, "error.agent.cannot-sign-up.invalid")

  class ViewTest(hasError: Boolean = false, isAgent: Boolean = true) {
    val page = view(
      if (hasError) {
  ...
  object CannotSignUpMessages {
    ...
    val formHeading = "Would you like to sign up your client for the next tax year?"
