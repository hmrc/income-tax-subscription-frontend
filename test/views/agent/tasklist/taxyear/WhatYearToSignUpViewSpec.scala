  ...
  "what year to sign up" must {
    ...
    "have a form" which {
      ...
      "has a save and continue button" in {
        form.getGovukSubmitButton.text mustBe WhatYearToSignUp.saveAndContinue
      }

      "has a continue button" in {
        val button: Element = form.selectHead(".govuk-button--secondary")
        button.text mustBe WhatYearToSignUp.saveAndComeBackLater
        button.attr("href") mustBe controllers.agent.tasklist.routes.ProgressSavedController.show(location = Some("select-tax-year")).url
      }
    }
  }
