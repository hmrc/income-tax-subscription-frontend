...
class ThrottleStartOfJourneyIndividualViewSpec extends ViewSpec {
  ...
  lazy val postAction: Call = controllers.usermatching.routes.HomeController.index
  ...
  "business individual start of throttle view" must {
    ...
    "have line 1" in {
      document1.mainContent.selectNth("p",1).text() mustBe line1
    }
    "have line 2" in {
      document1.mainContent.selectNth("p",2).text() mustBe line2
    }
    "have a continue button that says try again" in {
      document1.mainContent.selectHead(".form-group").text() mustBe tryAgain
    }
    ...
    "have a form that continues" in {
      document1.select("main").select("form").attr("action") mustBe controllers.usermatching.routes.HomeController.index.url
    }
