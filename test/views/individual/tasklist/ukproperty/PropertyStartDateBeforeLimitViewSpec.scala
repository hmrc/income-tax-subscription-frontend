...
class PropertyStartDateBeforeLimitViewSpec extends ViewSpec {

  "PropertyStartDateBeforeLimit" must {
    "use the correct template" when {
      ...
      "there is an error" in new TemplateViewTest(
        ...
        error = Some(FormError(
          key = PropertyStartDateBeforeLimitForm.startDateBeforeLimit,
          message = "error.property.start-date-before-limit.invalid",
      ...
      "has a yes no radio button question" when {
        "there is no error" in {
          form().mustHaveYesNoRadioInputs("fieldset")(
            name = PropertyStartDateBeforeLimitForm.startDateBeforeLimit,
      ...
      "has a button group" which {
        ...
        "has a save and come back later button" in {
          ...
          saveAndComeBackLater.attr("href") mustBe controllers.individual.tasklist.routes.ProgressSavedController.show(
            location = Some("uk-property-start-date-before-limit")
          ).url
        }
      }
    }
  ...
  lazy val propertyStartDateBeforeLimit: PropertyStartDateBeforeLimit = app.injector.instanceOf[PropertyStartDateBeforeLimit]

  def view(hasError: Boolean = false): HtmlFormat.Appendable = propertyStartDateBeforeLimit(
    startDateBeforeLimitForm = if (hasError) {
      PropertyStartDateBeforeLimitForm.startDateBeforeLimitForm.bind(Map.empty[String, String])
    } else {
      PropertyStartDateBeforeLimitForm.startDateBeforeLimitForm
    },
    postAction = testCall,
    backUrl = testBackUrl
  )
  ...
  object Messages {
    val caption: String = "Your UK property"
    val heading: String = s"Did you start getting this income before 6 April ${AccountingPeriodUtil.getStartDateLimit.getYear}?"
    val error: String = s"Select ‘Yes’ if you started getting income from UK property before 6 April ${AccountingPeriodUtil.getStartDateLimit.getYear}"
