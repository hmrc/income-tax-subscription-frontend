    ...
    "display property details" when {
      "all the answers have been completed" which {
        assertRow(
          ...
          changeLink = controllers.individual.tasklist.ukproperty.routes.PropertyStartDateController.show(editMode = true).url
        )
        assertRow(
          document(viewModel = completeCashProperty),
          ...
          changeLink = controllers.individual.tasklist.ukproperty.routes.PropertyAccountingMethodController.show(editMode = true).url
        )
        assertRow(
        ...
        "have a save and come back later button" in {
          ...
          buttonLink.attr("href") mustBe
            controllers.individual.tasklist.routes.ProgressSavedController.show().url + "?location=uk-property-check-your-answers"
        }
