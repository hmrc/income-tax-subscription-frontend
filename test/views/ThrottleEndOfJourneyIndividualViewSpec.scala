...
import assets.MessageLookup.ThrottleEndofJourney._
...
class ThrottleEndOfJourneyIndividualViewSpecViewSpec extends ViewSpec {
  ...
  "business individual end of journey throttle view" must {
    ...
    "have a form that continues" in {
      document1.select("main").select("form").attr("action") mustBe controllers.individual.business.routes.TaskListController.show().url
    }
