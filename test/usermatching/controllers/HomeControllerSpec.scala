...
import core.utils.{HttpResult, TestConstants}
...
import usermatching.services.OptionalIdentifiers
      ...
      "the user does not already have an MTDIT subscription on ETMP" when {
        ...
        "the user is not eligible" should {
          "redirect to the Not eligible page" in {
            ...
            val result = await(TestHomeController().index(fakeRequest))
            status(result) mustBe SEE_OTHER
            redirectLocation(result) mustBe Some(eligibilityRoutes.NotEligibleForIncomeTaxController.show().url)
          }
        }
      }
