...
class WhenDoYouWantToStartControllerSpec extends ControllerBaseSpec
  ...
  override val authorisedRoutes: Map[String, Action[AnyContent]] = Map(
    "show" -> TestWhenDoYouWantToStartController.show(isEditMode = false),
    "submit" -> TestWhenDoYouWantToStartController.submit(isEditMode = false)
  )
  ...
  "show" should {
    "display the When Do You Want To Start view with pre-saved tax year option and return OK (200)" when {
      "there is a pre-saved tax year option in Subscription Details " in {
        mockView()
        mockFetchSelectedTaxYear(Some(AccountingYearModel(Current)))

        val result = await(TestWhenDoYouWantToStartController.show(isEditMode = false)(subscriptionRequest))

        status(result) must be(Status.OK)
      }
    }

    "display the When Do You Want To Start view with empty form and return OK (200)" when {
    ...
    "the user is allowed to select a tax year" when {
      "they have not previously selected a tax year" should {
        "return OK with the page content" in {
          ...
          val result: Future[Result] = TestWhenDoYouWantToStartController.show(isEditMode = false)(subscriptionRequest)
  ...
  "submit" when {
    ...
    def callSubmit(isEditMode: Boolean, accountingYear: AccountingYear): Future[Result] = {
      TestWhenDoYouWantToStartController.submit(isEditMode = isEditMode)(
