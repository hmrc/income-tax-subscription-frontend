...
class OptedOutControllerSpec extends ControllerBaseSpec with MockIdentifierAction {
  ...
  class TestController(noEnrolment: Boolean) extends OptedOutController(
  ...
  "show returns Ok" in {
    Seq(false, true).foreach { noEnrolment =>
      val result = new TestController(noEnrolment).show()(fakeRequest)
