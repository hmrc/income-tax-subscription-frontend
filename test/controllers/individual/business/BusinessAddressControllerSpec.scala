  ...
  "When registration is enabled" should {
    ...
    "BusinessAddressController.submit" should {
      ...
      "when editMode is true" in {
        ...
        status(result)
        redirectLocation(result) mustBe Some(controllers.individual.business.routes.BusinessAddressController.init(editMode = true).url)
      }

      "when editMode is false" in {
        val result = call(editMode = false)
        status(result)
        redirectLocation(result) mustBe Some(controllers.individual.business.routes.BusinessStartDateController.show().url)
      }
    }
