...
class BusinessStartDateControllerSpec extends ControllerBaseSpec
  ...
  "When registration is enabled" should {
    ...
    "The back url" should {
      ...
      s"When it not is not in edit mode, it should point to '${controllers.individual.business.routes.BusinessAddressController.show().url}'" in {
        val document = Jsoup.parse(contentAsString(result(option_yes)))
        document.select("#back").attr("href") mustBe controllers.individual.business.routes.BusinessAddressController.show().url
      }

      s"When it is in edit mode it should point to '${controllers.individual.subscription.routes.CheckYourAnswersController.show().url}'" in {
        val document = Jsoup.parse(contentAsString(resultEdit(option_no)))
        document.select("#back").attr("href") mustBe controllers.individual.subscription.routes.CheckYourAnswersController.show().url
      }
