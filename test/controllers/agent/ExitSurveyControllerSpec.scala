...
class ExitSurveyControllerSpec extends AgentControllerBaseSpec {
  ...
  "ExitSurveyController.submit" when {
    ...
    "received an empty request" should {
      ...
      s"redirect to '${controllers.agent.routes.ThankYouController.show().url}'" in {
        redirectLocation(result) mustBe Some(controllers.agent.routes.ThankYouController.show().url)
      }
    }
