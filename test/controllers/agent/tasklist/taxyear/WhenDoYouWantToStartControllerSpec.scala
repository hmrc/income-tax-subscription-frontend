...
import views.agent.mocks.MockWhenDoYouWantToStart
...
class WhenDoYouWantToStartControllerSpec extends ControllerSpec
  with MockWhenDoYouWantToStart
  ...
  private def testWhenDoYouWantToStartController(sessionData: SessionData = SessionData()) = new WhenDoYouWantToStartController(
  ...
  "show" should {
    "display the when-do-you-wat-to-start view with pre-saved tax year option and return OK (200)" when {
      "there is a pre-saved tax year option in Subscription Details " in {
        ...
        val result = testWhenDoYouWantToStartController().show(isEditMode = false)(request)

        status(result) must be(Status.OK)
      }
    }

    "display the when-do-you-wat-to-start view with empty form and return OK (200)" when {
  ...
  "submit" when {
    def callSubmit(isEditMode: Boolean, taxYear: AccountingYear = Current, sessionData: SessionData = SessionData()): Future[Result] = testWhenDoYouWantToStartController(sessionData).submit(isEditMode = isEditMode)(
      request.withMethod("POST").withFormUrlEncodedBody(AccountingYearForm.accountingYear -> taxYear.toString)
    )

    def callSubmitWithErrorForm(isEditMode: Boolean): Future[Result] = testWhenDoYouWantToStartController().submit(isEditMode = isEditMode)(
    ...
    "there was a problem saving the tax year selection" must {
      "throw an internal server exception" in {
        ...
        intercept[InternalServerException](await(callSubmit(isEditMode = false)))
          .message mustBe "[WhenDoYouWantToStartController][saveSelectedTaxYear] - Could not save accounting year"
      }
    }
  }
}
