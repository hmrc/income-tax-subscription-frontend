...
class ClientRemoveUkPropertyControllerSpec extends AgentControllerBaseSpec
  ...
  override val controllerName: String = "ClientRemoveUkPropertyController"
  ...
  "show" when {
    ...
    "the save & retrieve feature switch is disabled" must {
      "return a not found exception" in withController { controller =>
        ...
        exception.message mustBe "[ClientRemoveUkPropertyController][show] - S&R feature switch is disabled"
      }
    }
  }
  ...
  "submit" when {
    ...
    "the save & retrieve feature switch is disabled" must {
      "return a not found exception" in withController { controller =>
        ...
        exception.message mustBe "[ClientRemoveUkPropertyController][submit] - S&R feature switch is disabled"
      }
    }
  }

  private def withController(testCode: ClientRemoveUkPropertyController => Any): Unit = {
    val view = mock[ClientRemoveUkProperty]

    when(view(any(), any(), any())(any(), any())).thenReturn(HtmlFormat.empty)

    val controller = new ClientRemoveUkPropertyController(
      mockAuditingService,
      mockAuthService,
