...
class RoutesSpec extends PlaySpec with OneAppPerTest {
  ...
  "The URL for the BusinessAccountingPeriodController.show action" should {
    s"be equal to $contextRoute/business/accounting-period" in {
      incometax.subscription.controllers.business.routes.BusinessAccountingPeriodPriorController.show().url must be(s"$contextRoute/business/accounting-period-prior")
    }
  }
  ...
  "The URL for the RegisterNextAccountingPeriod.show action" should {
    s"be equal to $contextRoute/business/register-next-accounting-period" in {
      incometax.subscription.controllers.business.routes.RegisterNextAccountingPeriodController.show().url must be(s"$contextRoute/business/register-next-accounting-period")
    }
  }
