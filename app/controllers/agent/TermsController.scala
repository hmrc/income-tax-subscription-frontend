...
class TermsController @Inject()(val baseConfig: BaseControllerConfig,
  ...
  def view(backUrl: String, taxEndYear: Int)(implicit request: Request[_]): Html =
    agent.views.html.terms(
      postAction = controllers.agent.routes.TermsController.submit(),
  ...
  def submit(isEditMode: Boolean = false): Action[AnyContent] = Authenticated.async { implicit request =>
    implicit user =>
      keystoreService.saveTerms(terms = true) map (
        _ => Redirect(controllers.agent.routes.CheckYourAnswersController.show()))
  }

  def backUrl(editMode: Boolean)(implicit request: Request[_]): Future[String] = {
    if (editMode) {
      controllers.agent.business.routes.BusinessAccountingPeriodDateController.show(editMode = true).url
    } else {
      keystoreService.fetchIncomeSource() flatMap {
        case Some(Business) => controllers.agent.business.routes.BusinessAccountingMethodController.show().url
        case Some(Both | Property) if isEnabled(AgentPropertyCashOrAccruals) => controllers.agent.business.routes.PropertyAccountingMethodController.show().url
