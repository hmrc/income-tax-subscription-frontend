    ...
    authorised().retrieve(affinityGroup and allEnrolments) {
      case Some(AffinityGroup.Agent) ~ allEnrolments =>
        allEnrolments.getEnrolment(Constants.hmrcAsAgent).flatMap { enrolment =>
          ...
          case Some(arn) => block(IdentifierRequest(request, arn))
          case None => Future.successful(Redirect(controllers.agent.matching.routes.NotEnrolledAgentServicesController.show))
        }
      case _ =>
