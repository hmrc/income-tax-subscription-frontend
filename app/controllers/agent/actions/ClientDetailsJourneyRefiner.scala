  ...
  override protected def refine[A](request: IdentifierRequest[A]): Future[Either[Result, IdentifierRequest[A]]] = {
    request.session.get(ITSASessionKeys.JourneyStateKey)
      ...
      case state@(None | Some(SignPosted | ConfirmedClient)) =>
        logger.info(s"[ClientDetailsJourneyRefiner][refine] - Incorrect user state, current: ${state.map(_.key)}, sending to add another client")
        Future.successful(Left(Redirect(controllers.agent.routes.AddAnotherClientController.addAnother())))
      case Some(Confirmation) =>
        logger.info(s"[ClientDetailsJourneyRefiner][refine] - Incorrect user state, current: ${Confirmation.key}, sending to confirmation page")
