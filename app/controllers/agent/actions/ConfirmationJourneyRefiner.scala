...
import models.JourneyStep.{ClientDetails, Confirmation, ConfirmedClient, SignPosted}
...
@Singleton
class ConfirmationJourneyRefiner @Inject()(utrService: UTRService,
  ...
  override protected def refine[A](request: IdentifierRequest[A]): Future[Either[Result, ConfirmedClientRequest[A]]] = {
    ...
    request.session.get(ITSASessionKeys.JourneyStateKey)
      ...
      case state@(None | Some(ClientDetails | SignPosted)) =>
        logger.info(s"[ConfirmationJourneyRefiner][refine] - Incorrect user state, current: ${state.map(_.key)}, sending to cannot go back page")
      ...
      case Some(ConfirmedClient) =>
        logger.info(s"[ConfirmationJourneyRefiner][refine] - Incorrect user state, current: ${ConfirmedClient.key}, showing a not found page")
