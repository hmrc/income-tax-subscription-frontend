...
@Singleton
class ConfirmationJourneyRefiner @Inject()(implicit val executionContext: ExecutionContext) extends ActionRefiner[IdentifierRequest, IdentifierRequest] {
  ...
  override protected def refine[A](request: IdentifierRequest[A]): Future[Either[Result, IdentifierRequest[A]]] = {
    request.session.get("JourneyStep").map(JourneyStep.fromString) match {
      ...
      case Some(ClientDetails) => Future.successful(Left(NotFound))
      case Some(ConfirmedClient) => Future.successful(Left(NotFound))
      case Some(Confirmation) => Future.successful(Right(request))
    }
  }
