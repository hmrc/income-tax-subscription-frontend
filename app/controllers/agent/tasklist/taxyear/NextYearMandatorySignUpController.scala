...
class NextYearMandatorySignUpController @Inject()(nextYearMandatorySignUp:NextYearMandatorySignUp,
  ...
  def view(accountingYearForm: Form[AccountingYear], isEditMode: Boolean)
    ...
    nextYearMandatorySignUp(
      ...
      postAction = controllers.agent.tasklist.taxyear.routes.NextYearMandatorySignUpController.submit(editMode = isEditMode),
  ...
  private def saveSelectedTaxYear(accountingYear: AccountingYear)
    ...
    subscriptionDetailsService.saveSelectedTaxYear(request.reference, AccountingYearModel(accountingYear)) map {
      ...
      case Left(_) => throw new InternalServerException("[NextYearMandatorySignUpController][saveSelectedTaxYear] - Could not save accounting year")
    }
  }
}
