...
class AgentNotAuthorisedController @Inject()(val baseConfig: BaseControllerConfig,
  ...
  def show: Action[AnyContent] = Authenticated { implicit request =>
    implicit user =>
      Ok(agent.views.html.agent_not_authorised(postAction = controllers.agent.routes.AgentNotAuthorisedController.submit()))
  }

  def submit: Action[AnyContent] = Authenticated { implicit request =>
    implicit user =>
      Redirect(controllers.agent.routes.HomeController.index()).withJourneyState(AgentUserMatched).clearUserDetails
  }
