...
class BusinessAddressController @Inject()(val baseConfig: BaseControllerConfig,
  ...
  private[controllers] def callbackUrl(editMode: Boolean)(implicit request: Request[AnyContent]): String =
    controllers.individual.business.routes.BusinessAddressController.callBack(editMode, "").absoluteURL().replace("""\?*.+$""", "")
              ...
              keystoreService.saveBusinessAddress(address).map {
                _ =>
                  if (editMode)
                    ...
                    Redirect(controllers.individual.business.routes.BusinessStartDateController.show())
              }
            case _ =>
  ...
  def view(address: Address, backUrl: String, isEditMode: Boolean)(implicit request: Request[_]): Html =
    views.html.individual.incometax.business.edit_business_address(
      ...
      backUrl,
      editUrl = if (isEditMode) None else Some(controllers.individual.business.routes.BusinessAddressController.init().url)
    )

  def show(editMode: Boolean): Action[AnyContent] = Authenticated.async { implicit request =>
    implicit user =>
      keystoreService.fetchBusinessAddress() map {
        case Some(address) => Ok(view(address, controllers.individual.business.routes.BusinessPhoneNumberController.show().url, editMode))
        case None => Redirect(controllers.individual.business.routes.BusinessAddressController.init())
      }
  }
  ...
  def submit(editMode: Boolean): Action[AnyContent] = Authenticated.async { implicit request =>
    implicit user =>
      if (editMode)
        Future.successful(Redirect(controllers.individual.business.routes.BusinessAddressController.init(editMode = true)))
      else
        Future.successful(Redirect(controllers.individual.business.routes.BusinessStartDateController.show()))
  }
