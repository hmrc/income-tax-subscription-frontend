...
class OverseasPropertyCheckYourAnswersController @Inject()(val view: OverseasPropertyCheckYourAnswers,
  ...
  def submit: Action[AnyContent] = Authenticated.async { implicit request =>
    implicit user =>
      if (isEnabled(SaveAndRetrieve)) {
        withProperty { property =>
          subscriptionDetailsService.saveOverseasProperty(property.copy(confirmed = true)).map { _ =>
  ...
  private def withProperty(f: OverseasPropertyModel => Future[Result])(implicit hc: HeaderCarrier) =
    subscriptionDetailsService.fetchOverseasProperty.flatMap {
