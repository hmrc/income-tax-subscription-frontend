...
@Singleton
class ClaimEnrolmentConfirmationJourneyRefiner @Inject()(implicit val executionContext: ExecutionContext)
  ...
  override protected def refine[A](request: IdentifierRequest[A]): Future[Either[Result, IdentifierRequest[A]]] = {
    ...
    request.session.get(ITSASessionKeys.JourneyStateKey)
      ...
      case Some(ClaimEnrolmentConfirmation) =>
        Future.successful(Right(request))
      case state@(None | Some(PreSignUp | SignUp | Confirmation | ClaimEnrolment)) =>
        logger.info(s"[Individual][ClaimEnrolmentConfirmationJourneyRefiner] - Incorrect user state, current: ${state.map(_.key)}. Sending to home")
