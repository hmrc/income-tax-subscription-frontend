  ...
  def backUrl(isEditMode: Boolean)(implicit request: Request[_]): Future[String] = {
    ...
    if (isEditMode)
      ...
      keystoreService.fetchOtherIncome().map {
        case Some(OtherIncomeModel(`option_yes`)) => _root_.controllers.routes.OtherIncomeErrorController.showOtherIncomeError().url
        case _ => _root_.controllers.routes.OtherIncomeController.showOtherIncome().url
      }
  }
