...
class BusinessStartDateController @Inject()(val baseConfig: BaseControllerConfig,
  ...
  def view(businessStartDateForm: Form[BusinessStartDateModel], isEditMode: Boolean)(implicit request: Request[AnyContent]): Html =
    incometax.subscription.views.html.business.business_start_date(
      businessStartDateForm = businessStartDateForm,
      postAction = incometax.subscription.controllers.business.routes.BusinessStartDateController.submit(editMode = isEditMode),
      isEditMode,
  ...
  def backUrl(isEditMode: Boolean): String =
    if (isEditMode)
      ...
      incometax.subscription.controllers.business.routes.BusinessAddressController.show().url
