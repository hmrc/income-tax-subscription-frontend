...
class BusinessAccountingMethodController @Inject()(val baseConfig: BaseControllerConfig,
  ...
  def view(accountingMethodForm: Form[AccountingMethodModel], isEditMode: Boolean)(implicit request: Request[_]): Html =
    incometax.subscription.views.html.business.accounting_method(
      accountingMethodForm = accountingMethodForm,
      postAction = incometax.subscription.controllers.business.routes.BusinessAccountingMethodController.submit(editMode = isEditMode),
      isEditMode,
  ...
  def backUrl(isEditMode: Boolean): String =
    if (isEditMode)
      ...
      incometax.subscription.controllers.business.routes.BusinessAccountingPeriodDateController.show().url
