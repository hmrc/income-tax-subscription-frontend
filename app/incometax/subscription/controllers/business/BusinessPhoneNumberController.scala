...
class BusinessPhoneNumberController @Inject()(val baseConfig: BaseControllerConfig,
  ...
  def view(businessPhoneNumberForm: Form[BusinessPhoneNumberModel], isEditMode: Boolean)(implicit request: Request[_]): Html =
    incometax.subscription.views.html.business.business_phone_number(
      businessPhoneNumberForm = businessPhoneNumberForm,
      postAction = incometax.subscription.controllers.business.routes.BusinessPhoneNumberController.submit(editMode = isEditMode),
      isEditMode,
        ...
        phoneNumber => {
          keystoreService.saveBusinessPhoneNumber(phoneNumber) map (_ =>
            if (isEditMode)
              ...
              Redirect(incometax.subscription.controllers.business.routes.BusinessAddressController.show())
            )
        }
      )
  }
  ...
  def backUrl(isEditMode: Boolean): String =
    if (isEditMode)
      ...
      incometax.subscription.controllers.business.routes.BusinessNameController.show().url
