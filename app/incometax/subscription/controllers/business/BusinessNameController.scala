...
class BusinessNameController @Inject()(val baseConfig: BaseControllerConfig,
  ...
  def view(businessNameForm: Form[BusinessNameModel], isEditMode: Boolean)(implicit request: Request[AnyContent]): Future[Html] =
    backUrl(isEditMode).map { backUrl =>
      incometax.subscription.views.html.business.business_name(
        businessNameForm = businessNameForm,
        postAction = incometax.subscription.controllers.business.routes.BusinessNameController.submit(editMode = isEditMode),
        ...
        businessName =>
          keystoreService.saveBusinessName(businessName) map (_ =>
            if (isEditMode)
              ...
              Redirect(incometax.subscription.controllers.business.routes.BusinessPhoneNumberController.show())
            else
              Redirect(incometax.subscription.controllers.business.routes.BusinessAccountingPeriodPriorController.show())
            )
      )
  }
