...
class BusinessAddressController @Inject()(val baseConfig: BaseControllerConfig,
  ...
  private[controllers] def callbackUrl(editMode: Boolean)(implicit request: Request[AnyContent]): String =
    incometax.subscription.controllers.business.routes.BusinessAddressController.callBack(editMode,"").absoluteURL().replace("""\?*.+$""", "")
              ...
              keystoreService.saveBusinessAddress(address).map {
                _ =>
                  if (editMode)
                    ...
                    Redirect(incometax.subscription.controllers.business.routes.BusinessStartDateController.show())
              }
            case _ =>
  ...
  def view(address: Address, backUrl: String, isEditMode: Boolean)(implicit request: Request[_]): Html =
    incometax.subscription.views.html.business.edit_business_address(
      address,
      incometax.subscription.controllers.business.routes.BusinessAddressController.submit(editMode = isEditMode),
      backUrl,
      editUrl = if (isEditMode) None else Some(incometax.subscription.controllers.business.routes.BusinessAddressController.init().url)
    )

  def show(editMode: Boolean): Action[AnyContent] = Authenticated.async { implicit request =>
    implicit user =>
      keystoreService.fetchBusinessAddress() map {
        case Some(address) => Ok(view(address, incometax.subscription.controllers.business.routes.BusinessPhoneNumberController.show().url, editMode))
        case None => Redirect(incometax.subscription.controllers.business.routes.BusinessAddressController.init())
      }
  }
  ...
  def submit(editMode: Boolean): Action[AnyContent] = Authenticated.async { implicit request =>
    implicit user =>
      if (editMode)
        Future.successful(Redirect(incometax.subscription.controllers.business.routes.BusinessAddressController.init(editMode = true)))
      else
        Future.successful(Redirect(incometax.subscription.controllers.business.routes.BusinessStartDateController.show()))
  }
