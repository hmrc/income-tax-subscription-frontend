...
class BusinessAccountingPeriodPriorController @Inject()(val baseConfig: BaseControllerConfig,
  ...
  def view(accountingPeriodPriorForm: Form[AccountingPeriodPriorModel], isEditMode: Boolean)(implicit request: Request[_]): Future[Html] =
    incometax.subscription.views.html.business.accounting_period_prior(
        accountingPeriodPriorForm = accountingPeriodPriorForm,
        postAction = incometax.subscription.controllers.business.routes.BusinessAccountingPeriodPriorController.submit(editMode = isEditMode),
        backUrl = backUrl,
  ...
  def yes(implicit request: Request[_]): Future[Result] = Redirect(incometax.subscription.controllers.business.routes.RegisterNextAccountingPeriodController.show())

  def no(implicit request: Request[_]): Future[Result] = Redirect(incometax.subscription.controllers.business.routes.BusinessAccountingPeriodDateController.show())

  lazy val backUrl = incometax.subscription.controllers.business.routes.BusinessNameController.show().url
