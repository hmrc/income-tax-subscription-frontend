...
object BusinessStartDateForm {
  ...
  val dateValidation: Constraint[(Option[String], Option[String], Option[String])] = constraint[(Option[String], Option[String], Option[String])] {
    case (day, month, year) => {
      lazy val invalidDate = Invalid("error.business.date.empty")
  ...
  val validateDate: Constraint[(Option[String], Option[String], Option[String])] = constraint[(Option[String], Option[String], Option[String])] {
    case (None, None, None) => Invalid("error.business.date.empty")
    case (Some(_), None, None) => Invalid("error.business.month.year.empty")
    case (None, None, Some(_)) => Invalid("error.business.day.month.empty")
    case (None, Some(_), None) => Invalid("error.business.day.year.empty")
    case (Some(_), Some(_), None) => Invalid("error.business.year.empty")
    case (None, Some(_), Some(_)) => Invalid("error.business.day.empty")
    case (Some(_), None, Some(_)) => Invalid("error.business.month.empty")
