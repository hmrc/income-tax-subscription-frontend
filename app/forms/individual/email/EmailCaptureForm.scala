...
object EmailCaptureForm {
  ...
  val emailIsPresent: Constraint[String] = nonEmpty(s"error.individual.$formKey.empty")
  val emailLength: Constraint[String] = maxLength(emailMaxLength, s"error.individual.$formKey.max-length")
  val emailIsCorrectPattern: Constraint[String] = pattern(emailPattern, "email.regex", s"error.individual.$formKey.invalid")
