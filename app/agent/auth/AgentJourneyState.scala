...
object AgentJourneyState {
  ...
  implicit class SessionFunctions(session: Session) {
    ...
    def setAuthorisedAgentFlag(authorisedFlag: Boolean): Unit = session + ITSASessionKeys.AuthorisedAgentKey -> authorisedFlag.toString
  }
  ...
  implicit class RequestFunctions(request: Request[_]) {
    ...
    def isAuthorisedAgent: Boolean = request.session.isAuthorisedAgent
  }
  ...
  implicit class ResultFunctions(result: Result) {
    ...
    def setAuthorisedAgentFlag(authorisedFlag: Boolean): Unit = setAuthorisedAgentFlag(authorisedFlag)
  }
