...
class NotEnrolledAgentServicesController @Inject()(val baseConfig: BaseControllerConfig,
  ...
  val show: Action[AnyContent] = Action.async {
    implicit request =>
      Future.successful(Ok(agent.views.html.not_enrolled_agent_services(getAction =
        core.controllers.SignOutController.signOut(routes.NotEnrolledAgentServicesController.show())
      )))
  }
