...
object PostMandationStatusParser {
  type PostMandationStatusResponse = Either[ErrorModel, MandationStatusModel]

  implicit val mandationStatusResponseHttpReads: HttpReads[PostMandationStatusResponse] =
    (_: String, _: String, response: HttpResponse) => {
      response.status match {
        case OK => response.json.validate[MandationStatusModel] match {
