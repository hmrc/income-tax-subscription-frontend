...
class CannotSignUpThisYearControllerISpec extends ComponentSpecBase {
  ...
  "POST /error/cannot-sign-up-for-current-year" when {
    ...
    "the user submits 'no'" should {
      ...
      s"return a redirect to ${controllers.individual.controllist.routes.DeclinedSignUpNextYearController.show.url}" in {
        Given("I setup the wiremock stubs")
        AuthStub.stubAuthSuccess()
        ...
        result must have(
          httpStatus(SEE_OTHER),
          redirectURI(controllers.individual.controllist.routes.DeclinedSignUpNextYearController.show.url)
        )
      }
    }
